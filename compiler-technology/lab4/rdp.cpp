...
void advanceTok(){ if(idx < (int)tok.size()) ++idx; }

void match(const string &t){
    if(cur() == t) advanceTok();
    else {
        cerr << "Error: expected '" << t << "' but found '" << cur() << "'. Skipping token.\n";
        if(cur() != "$") advanceTok();
    }
}
...
void F(){
    if(cur() == "id"){
        ...
        cerr << "Error: expected 'id' or '(', but found '" << cur() << "'. Skipping token.\n";
        if(cur() != "$") advanceTok();
    }
}
...
int main(){
    ...
    E();
    if(cur() == "$"){
        cout << "Parsing finished: input accepted (or no unrecoverable errors encountered).\n";
    } else {
